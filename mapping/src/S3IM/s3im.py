...
class S3IM(torch.nn.Module):
    ...
    def forward(self, src_vec, tar_vec):
        ...
        if tar_all.shape[0] % (self.patch_height * self.patch_width * self.repeat_time) == 0:
            ...
            tar_patch = tar_all[:(self.patch_height - 1) * self.patch_width * self.repeat_time].permute(1, 0).reshape(1, 3, self.patch_height - 1, self.patch_width * self.repeat_time)
            src_patch = src_all[:(self.patch_height - 1) * self.patch_width * self.repeat_time].permute(1, 0).reshape(1, 3, self.patch_height - 1, self.patch_width * self.repeat_time)
