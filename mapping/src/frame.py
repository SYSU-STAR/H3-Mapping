...
class RGBDFrame(nn.Module):
    def __init__(self, fid, rgb, depth, K, offset, ref_pose=None, img_raw=None) -> None:
        super().__init__()
        self.stamp = fid
        ...
        if type(K) != torch.Tensor:
            self.K = torch.FloatTensor(K).cuda()
        else:
            self.K = K
        self.img_raw = img_raw
        self.noninflate_voxel_idx_points, self.new_covered_voxels_idx = None, None
    ...
    def get_rays(self, w=None, h=None, K=None):
        ...
        rays_d = torch.stack(
            [(ix.cuda() - K[0, 2]) / K[0, 0],
             (iy.cuda() - K[1, 2]) / K[1, 1],
             torch.ones_like(ix).cuda()], -1).float()  # camera coordinate
        return rays_d
    ...
    @torch.no_grad()
    def get_points_and_mask(self):
        return self.points, self.valid_mask
