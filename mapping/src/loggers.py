...
class BasicLogger:
    ...
    def log_ckpt(self, mapper, name):
        ...
        map_state = {}
        for k, v in mapper.map_states.items():
    ...
    def log_images(self, ind, gt_rgb, gt_depth, rgb, depth):
        ...
        if self.for_eva:
            ...
            if gt_color_np[(gt_depth_np != 0.0) * (depth_np > 1e-4)].shape[0] == 0:
                ...
                plt.imsave(osp.join(self.img_dir_rgb_err_render, "color_residual{:06d}.jpg".format(ind)),
                           color_residual.mean(axis=-1), vmin=0, vmax=1)
                return None, None, None

            ssim = structural_similarity(gt_color_np[(gt_depth_np != 0.0) * (depth_np > 1e-4)],
                                         color_np[(gt_depth_np != 0.0) * (depth_np  > 1e-4)], channel_axis=-1,data_range=1)
            psnr = peak_signal_noise_ratio(gt_color_np[(gt_depth_np != 0.0) * (depth_np  > 1e-4)],
                                           color_np[(gt_depth_np != 0.0) * (depth_np  > 1e-4)])
            depth_L1_err = np.mean(depth_residual[(gt_depth_np != 0.0) * (depth_np  > 1e-4)])
            color_np = cv2.cvtColor(color_np, cv2.COLOR_RGB2BGR)
            cv2.imwrite(osp.join(self.img_dir_rgb_render, "frame{:06d}.jpg".format(ind)), color_np * 255)
            cv2.imwrite(osp.join(self.img_dir_depth_render, "depth{:06d}.png".format(ind)),
                        (depth_np * 1000).astype(np.uint16))  # change to mm
