...
@torch.no_grad()
def eval_points(sdf_network, sampled_xyz, map_states, chunk_samples):
    def get_scores_once(sampled_xyz, chunk_samples_per=None):
        sampled_xyz = sampled_xyz.reshape(-1, 3)

        if sampled_xyz.shape[0] == 0:
            return
        color = sdf_network.get_color(sampled_xyz, map_states, chunk_samples_per)
    ...
    for i in range(0, sampled_xyz.size(0), chunk_size):
        with torch.cuda.amp.autocast():
            if len(chunk_samples) == 0:
                score_once = get_scores_once(sampled_xyz[i: i + chunk_size])
            else:
                chunk_samples_per = {}
                chunk_samples_per["sampled_point_voxel_idx"] = chunk_samples["sampled_point_voxel_idx"][i: i + chunk_size].cuda()
                score_once = get_scores_once(sampled_xyz[i: i + chunk_size].cuda(), chunk_samples_per)
...
def sdf2weights(sdf_in, trunc, z_vals, sample_mask_per, new_sdf_strategy, gt_detph = None):
    ...
    if new_sdf_strategy:
        ...
        inds_top2 = torch.argsort(mask,descending=True,stable=True,axis=1)[:,:2]
        inds = inds_top2[:,0:1] + 1 # +1 means to select latter point of the +/- change; inds is the latter, inds2 is the former
        inds2 = inds_top2[:,1:2]
        inds2[inds2 == 0] = mask.shape[1] - 1
        # make the mask_before_second_surface not before the gt_depth when training
        if gt_detph is not None:
            ind_gt = torch.argmax((gt_detph.unsqueeze(-1) < z_vals).int(), axis=1)
            inds2[inds2[:,0] <= ind_gt] = mask.shape[1] - 1
        mask_before_second_surface = torch.ones(mask.shape[0], mask.shape[1] + 1).cuda()
        if mask.shape[1] != 1: # if only two samples, inds2 is invalid
            column_indices = torch.arange(mask_before_second_surface.size(1)).cuda()
            mask_before_second_surface[column_indices.unsqueeze(0) >= inds2] = 0
            mask_before_second_surface[:,-1] = 0
    else:
        inds = torch.argmax(mask, axis=1)
        inds = inds[..., None]
        mask_before_second_surface = None

    # add to avoid no sign change in the ray
    sdf_mask_all_zero = sdf_in[(mask == 0).all(-1)]
    inds[(mask == 0).all(-1)] = sdf_mask_all_zero.abs().argmin(axis=1)[..., None]
    ...
    if new_sdf_strategy:
        # avoid two surface in the truc region
        tmp = torch.arange(0,mask.shape[1]).cuda().unsqueeze(0)
        mask2 = tmp <= (inds + 2)
        mask = mask * mask2
    # mask truncation and mask not hit voxel
    weights = weights * mask * sample_mask_per
    return weights / (torch.sum(weights, dim=-1, keepdims=True) + 1e-8), z_min, mask_before_second_surface
...
def render_rays(
    ...
    new_sdf_strategy = map_states["new_sdf_strategy"]
    ...
    for i in range(0, num_points, chunk_size):
        ...
        chunk_outputs['sdf'] = chunk_outputs['sdf'][:, 0] + chunk_inputs['sdf_priors'][:, -1]
    ...
    weights, z_min, _ = sdf2weights(sdf, truncation, z_vals, sample_mask, new_sdf_strategy)
...
def bundle_adjust_frames(
    ...
    for epoch in range(num_iterations):
        ...
        with torch.cuda.amp.autocast():
            ...
            z_vals = samples["sampled_point_depth"][epoch * N_rays: (epoch + 1) * N_rays][ray_mask_per[0]]  # (N_rays, N_samples_every_ray), the depth from cam

            gt_depth = depth_samples_all[:, epoch * N_rays: (epoch + 1) * N_rays].clone()
            gt_color = rgb_samples_all[:, epoch * N_rays: (epoch + 1) * N_rays].clone()
            weights, z_min, mask_before_second_surface = sdf2weights(sdf, truncation, z_vals, sample_mask_per, new_sdf_strategy, gt_depth[ray_mask_per])
            ...
            final_outputs = {
                ...
                "mask_before_second_surface": mask_before_second_surface,
            }
            loss = loss_criteria(final_outputs, (gt_color, gt_depth))
