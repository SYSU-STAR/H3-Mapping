...
@torch.no_grad()
def ray_intersect(ray_start, ray_dir, flatten_centers, flatten_children, voxel_size, max_hits=None, max_distance=10.0, jcx_max_hits=None):
    # ray-voxel intersection
    if jcx_max_hits is None:
        max_hits_temp = 50
    else:
        max_hits_temp = jcx_max_hits
    ...
    pts_idx[(max_depth - min_depth).abs() < 1e-3] = -1
    # sort the depths
...
def ray_sample(intersection_outputs, step_size=0.01, fixed=False):
    dists = (
            ...
            intersection_outputs["min_depth"]
                  ...
                  0)  # dist calculates the distance of the ray within the voxel. The following sampling essentially means that rays with shorter distances within a voxel are sampled less frequently.
