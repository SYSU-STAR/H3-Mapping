...
class _module_function(torch.autograd.Function):
	...
	def backward(ctx, doutput):
		...
		# print("_module_function_bwd_id: ", ctx.prime_id)
		input_grad, params_grad = _module_function_backward.apply(ctx, doutput, input, params, output)

		return None, null_tensor_to_none(input_grad), null_tensor_to_none(params_grad), None, None


class _module_function_backward(torch.autograd.Function):
	@staticmethod
	def forward(ctx, ctx_fwd, doutput, input, params, output):
		# print("backward prime_id: ", ctx_fwd.prime_id)
		ctx.ctx_fwd = ctx_fwd
		ctx.save_for_backward(input, params, doutput)
		ctx.prime_id = ctx_fwd.prime_id
		with torch.no_grad():
			scaled_grad = doutput * ctx_fwd.loss_scale
			input_grad, params_grad = ctx_fwd.native_tcnn_module.bwd(ctx_fwd.native_ctx, input, params, output,
																	 scaled_grad, ctx.prime_id)
	...
	def backward(ctx, dinput_grad, dparams_grad):
		...
		return None, doutput_grad, input_grad, params_grad, None, None
...
class Module(torch.nn.Module):
	...
	def forward(self, x, prime_id=0):
		...
		x_padded = x if batch_size == padded_batch_size else torch.nn.functional.pad(x, [0, 0, 0,
...
class NetworkWithInputEncoding(Module):
	...
	def _native_tcnn_module(self):
		return _C.create_network_with_input_encoding(self.n_input_dims, self.n_output_dims, self.encoding_config,
													 self.network_config)
