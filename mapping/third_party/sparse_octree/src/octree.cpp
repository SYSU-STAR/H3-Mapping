...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor ,torch::Tensor ,torch::Tensor>  Octree::insert(torch::Tensor pts)
{
    if (root_ == nullptr)
    {
    ...
    int frame_voxel_structure_idx[points.size(0)][1];
...
//            all_keys.insert(key);
            ...
            if (l_it != surface_keys.end() && j==0)
            {
                ...
                auto l_it_structure = structure_keys.find(key);
                frame_voxel_structure_idx[i][0] = l_it_structure->second;
                continue;
            }
            ...
            for (int d = 1; d <= max_level_; edge /= 2, ++d)
            {
                ...
                if (is_surface && tmp->is_leaf_){
                    features_.set(surface_id, j, tmp->index_);
                }
                if (tmp->type_ == SURFACE && j == 0){
                    is_surface = true;
                    surface_id = tmp->index_;
                    features_.set(surface_id, j, tmp->index_);
                    frame_voxel_idx[i][0] = tmp->index_;
                    frame_voxel_structure_idx[i][0] = tmp->index_;
                    surface_keys.insert(std::pair < uint64_t, int > (key,tmp->index_));
                    structure_keys.insert(std::pair < uint64_t, int > (key,tmp->index_));
                }
                n = tmp;
            }
            }
    ...
    return std::make_tuple(torch::from_blob(voxel_.ptr(), {grid_num_,4}, dtype(torch::kFloat32)).clone(),
                           ...
                           torch::from_blob(frame_voxel_structure_idx, {points.size(0),1}, dtype(torch::kInt32)).clone());
}
