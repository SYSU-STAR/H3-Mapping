...
int main() {
  ...
  cv::Mat img = cv::imread("../images/bb.png");
 ...
 // print img[0,0]
  ...
  upm::SalientSegments segs_salient = elsed.detectSalient(img);
  std::cout << "ELSED detected: " << segs.size() << " (large) segments" << std::endl;
  std::cout << "ELSED detected salient: " << segs_salient.size() << " (large) segments" << std::endl;
