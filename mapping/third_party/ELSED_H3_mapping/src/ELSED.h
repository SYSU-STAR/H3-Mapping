...
struct ELSEDParams {
  ...
  std::vector<int> listJunctionSizes = {};
  // the minimum distance between two pc when they are coonsider as the nearby pc
...
class ELSED {
 public:
  ...
  SalientSegments detectSalient_pc_dist(const cv::Mat &image, const cv::Mat &pc_dist);
  ...
  void processImage_pc_dist(const cv::Mat &image, const cv::Mat &image_pc_dist);
  ...
  static LineDetectionExtraInfoPtr
  computeGradients_pc_dist(const cv::Mat &srcImg, const cv::Mat &pc_distImg, short gradientTh, float gradientTh_pc_dist);
