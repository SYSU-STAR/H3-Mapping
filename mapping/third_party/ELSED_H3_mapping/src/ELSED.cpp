...
SalientSegments ELSED::detectSalient_pc_dist(const cv::Mat &image, const cv::Mat &pc_dist) {
    processImage_pc_dist(image, pc_dist);
    // std::cout << "ELSED detected: " << salientSegments.size() << " salient segments" << std::endl;
    return salientSegments;
}
...
void ELSED::processImage_pc_dist(const cv::Mat &_image, const cv::Mat &_image_pc_dist) {
    // Check that the image is a grayscale image
    cv::Mat image;
    switch (_image.channels()) {
        case 3:
...
//    auto startTime = std::chrono::high_resolution_clock::now();
    ...
    if (params.ksize > 2) {
        cv::GaussianBlur(image, blurredImg, cv::Size(params.ksize, params.ksize), params.sigma);
//        std::cout << params.ksize << std::endl;
    } else {
        blurredImg = image;
    }
//    auto endTime = std::chrono::high_resolution_clock::now();
//    auto elapsedTime = std::chrono::duration_cast<std::chrono::nanoseconds>(endTime - startTime).count();
//    std::cout << "Elapsed time 1: " << elapsedTime << " ns" << std::endl;
//    startTime = std::chrono::high_resolution_clock::now();
    // Compute the input image derivatives
    imgInfo = computeGradients_pc_dist(blurredImg, _image_pc_dist, params.gradientThreshold, params.pc_dist_threshold);
//    endTime = std::chrono::high_resolution_clock::now();
//    elapsedTime = std::chrono::duration_cast<std::chrono::nanoseconds>(endTime - startTime).count();
//    std::cout << "Elapsed time 2: " << elapsedTime << " ns" << std::endl;
//    startTime = std::chrono::high_resolution_clock::now();
    bool anchoThIsZero;
    uint8_t anchorTh = params.anchorThreshold;
    do {
...
//    endTime = std::chrono::high_resolution_clock::now();
//    elapsedTime = std::chrono::duration_cast<std::chrono::nanoseconds>(endTime - startTime).count();
//    std::cout << "Elapsed time 3: " << elapsedTime << " ns" << std::endl;
}
...
LineDetectionExtraInfoPtr ELSED::computeGradients_pc_dist(const cv::Mat &srcImg, const cv::Mat &pc_distImg, short gradientTh, float gradientTh_pc_dist) {
    LineDetectionExtraInfoPtr dstInfo = std::make_shared<LineDetectionExtraInfo>();
    cv::Sobel(srcImg, dstInfo->dxImg, CV_16SC1, 1, 0, 3, 1, 0, cv::BORDER_REPLICATE);
    cv::Sobel(srcImg, dstInfo->dyImg, CV_16SC1, 0, 1, 3, 1, 0, cv::BORDER_REPLICATE);
    ...
    auto *pGrWO = dstInfo->gImgWO.ptr<int16_t>();
    auto *pDir = dstInfo->dirImg.ptr<uchar>();
    auto *pDispc = pc_distImg.ptr<float>();
    int16_t abs_dx, abs_dy, sum;
    const int totSize = nRows * nCols;
    for (i = 0; i < totSize; ++i) {
        // Absolute value
        ...
        abs_dy = UPM_ABS(pDY[i]);
        sum = abs_dx + abs_dy;
        // remove the invalid gradient (in two object)
        if (pDispc[i] > gradientTh_pc_dist) {
            sum = 0;
//            std::cout << pDispc[i] << std::endl;
        }
        // Divide by 2 the gradient
        pGrWO[i] = sum;
        pGr[i] = sum < gradientTh ? 0 : sum;
  ...
  for (const FullSegmentInfo &detectedSeg: drawer->getDetectedFullSegments()) {
    ...
    if (params.validate) {
      if (detectedSeg.getNumOfPixels() < 2) {
        ...
        if (s[0] == s[2] && s[1] == s[3]) {
            // If the endpoints are the same, the line equation is not defined
            valid = false;
        } else{
...
//            saliency = nOriInliers; // raw
            saliency = (float)nOriInliers / (float)(nOriInliers + nOriOutliers); // normalized

//            std::cout << "nOriInliers: " << nOriInliers << " nOriOutliers: " << nOriOutliers << "saliency: " << saliency << std::endl;
...
//      std::cout << "saliency: " << saliency << std::endl;
    }
    if (valid) {
