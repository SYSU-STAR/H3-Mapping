...
class ArgumentParserX(argparse.ArgumentParser):
    def __init__(self, vis, config_path, **kwargs):
        super().__init__(**kwargs)
        if config_path is None:
            self.add_argument("--config", type=str, default="configs/replica/room_0.yaml")
        else:
            self.add_argument("--config", type=str, default=config_path)
        if not vis:
...
def get_parser(vis=False, config_path=None):
    parser = ArgumentParserX(vis=vis,config_path=config_path)
