...
if __name__ == '__main__':
    ...
    first_frame = RGBDFrame(fid, rgb, depth, K, offset=args.mapper_specs['offset'], ref_pose=ref_pose,img_raw=img_raw).cuda()
    ...
    logger.img_dir_rgb_err_render = result_file + "/mapping_vis/color_residual"
    ...
    if not os.path.exists(logger.img_dir_rgb_err_render):
        os.makedirs(logger.img_dir_rgb_err_render)
    ...
    for i in tqdm(range(len(data_stream))):
        ...
        tracked_frame = RGBDFrame(fid, rgb, depth, K, offset=args.mapper_specs['offset'], ref_pose=ref_pose, img_raw=img_raw).cuda()
        ...
        print("ssim: ", ssim, "psnr: ", psnr, "depth_L1_err: ", depth_L1_err)
