...
def calc_3d_metric(args, align=True):
    ...
    if only_cal_has_vox:
        ...
        for batch_iter in tqdm(range(0, mesh_gt.vertices.shape[0], batch_size)):
            ...
            tmp = (dis.abs() < (args.mapper_specs["voxel_size"] / 2 + 1e-2)).all(dim=-1).any(dim=-1)
    ...
    np.savetxt(args.out_dir + "/3d_matrics.txt", np.array([accuracy_rec, completion_rec]))
    np.savetxt(args.out_dir + "/3d_matrics_std.txt",
               np.array([accuracy_rec_std, completion_rec_std]))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-only_cal_has_vox', action='store_true',
                        help='remove faces from a gt_mesh that are occluded in all RGB-D frames', default=False)
